/**
 * @brief Este archivo contiene las funciones para el envio del video
...
void * main_video(void *arg)
{
	...
	dir.sin_port = htons(client_rtp_port_2); /*Le asignamos el puerto correcto, el RTP*/

	/*Creamos el socket por donde enviar la informacion*/
	sockfd = crea_socket_RTP(server_rtp_port_2);
	if(sockfd == -1){
		fprintf(stderr,"Error al intentar abrir el socket RTP de video\n");
	...
	while((leidos = read(file, buff, ENV_VIDEO)) > 0){
		...
		if(estado_video == Seguir){
			...
			envia_paquete_RTP(sockfd, (struct sockaddr *)&dir, sizeof(struct sockaddr_in), 
					  	  sec, sec*TEMP, ssrc, buff, ENV_VIDEO);

			sec++;
            enviados++;

		}else if(estado_video == Parar){/*Si nos mandan parar, nos quedamos en el semaforo correspondiente*/
...
static int crea_hilos_RTCP(void)
{
	...
	f = fopen(VIDEO_FICH,"w");
	if(f == NULL) return -1;

	/*Creamos el socket RTCP*/
	if((sock_rtcp = crea_socket_RTCP(server_rtcp_port_2)) == -1){
		fclose(f);
		return -1;
	}
...
static void *escucha_rtcp(void *nada)
{
	...
	guardamos la informacion relevante en VIDEO_FICH y cambiamos
	...
	while(1){
		...
		if(escucha_comandoRTCP(sock_rtcp, (struct sockaddr *)&dir, &tam, &rec) == -1){/*Si hubo error al recibir el paquete*/
			fprintf(stderr,"Error recibiendo mensaje RTCP video\n");
			...
			if(rec.tipo == rr_t){/*Comprobamos que sea un RR*/
				...
				if(por_lost >= 0.2){/*Si se pierde mas del 30% de los paquetes*/
					tiempo+=VAR_TIEMPO;
				}else{/*Si se pierden menos del 30%*/
					tiempo-=VAR_TIEMPO;
				}

				/*escribimos la informacion en el fichero VIDEO_FICH*/
...
static void *envia_rtcp(void *nada)
{
    ...
    dir_rtcp.sin_port = htons(client_rtcp_port_2);
